...
`,
  js: `export class Utils {
  static mongoToGraphQL = (object, populate) => {
    const newO = {
      ...object,
    };
    if (populate) {
  ...
  static mapObjectsToGraphQLID = (objects, populate) => {
    return objects.map((o) => Utils.mongoToGraphQL(o, populate));
  };
  static CursorToGraphQLArray = async (cursor, populate) => {
    const list = await cursor.toArray();
    return Utils.mapObjectsToGraphQLID(list, populate);
  };
  static ManyToMany = async ({ db, collection, objects, pk, propertyName }) =>
    await Utils.CursorToGraphQLArray(
      await db.collection(collection).find({
        [pk]: {
          $in: objects
  ...
  static OneToMany = async ({ db, collection, objects, pk, propertyName }) =>
    await Utils.CursorToGraphQLArray(
      await db.collection(collection).find({
        [pk]: {
          $in: objects.map((o) => o[propertyName]),
